...
import { UserXIcon, BellIcon, ClockIcon, MessageSquareIcon, UserCheckIcon } from "lucide-react";
...
const NotificationsPage = () => {
  ...
  const incomingRequests = friendRequests?.pending || [];
  const acceptedRequests = friendRequests?.accepted || [];
  const rejectedFriendRequest = friendRequests?.rejected || [];
      ...
      <div className="container mx-auto max-w-4xl space-y-8">
        ...
        {isLoading ? (
          ...
          <>
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            <div className="avatar w-14 h-14 rounded-full overflow-hidden bg-base-300">
                              <img src={request.sender.profilePic.url} alt={request.sender.fullName} />
                            </div>
                            <div>
                      ...
                      <div className="card-body p-4">
                        <div className="flex items-start gap-3">
                          <div className="avatar mt-1 size-10 rounded-full overflow-hidden">
                            <img
                              src={notification.recepient.profilePic.url}
            ...
            {/* ACCEPTED REQS NOTIFICATONS */}
            {rejectedFriendRequest.length > 0 && (
              <section className="space-y-4">
                <h2 className="text-xl font-semibold flex items-center gap-2">
                  <UserXIcon className="h-5 w-5 text-success" />
                  Rejected Connections
                </h2>

                <div className="space-y-3">
                  {rejectedFriendRequest.map((notification) => (
                    <div key={notification._id} className="card bg-base-200 shadow-sm">
                      <div className="card-body p-4">
                        <div className="flex items-start gap-3">
                          <div className="avatar mt-1 size-10 rounded-full overflow-hidden">
                          ...
                          <div className="flex-1">
                            <h3 className="font-semibold">{notification.recepient.fullName}</h3>
                            <p className="text-sm my-1">
                              {notification.recepient.fullName} rejected your friend request
                            </p>
                            <p className="text-xs flex items-center opacity-70">
                              <ClockIcon className="h-3 w-3 mr-1" />
                              Recently
            ...
            {incomingRequests.length === 0 && acceptedRequests.length === 0 && rejectedFriendRequest.length === 0 && (
